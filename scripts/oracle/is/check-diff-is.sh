#!/usr/bin/env bash
# Exact Icelandic match-set diff: Java LT `check()` vs `lt-cli check -l is
# --lines`. Requires Docker for the pinned Java build. The Java side runs the
# default variant (is).
#
# Usage: scripts/oracle/is/check-diff-is.sh <sentences.txt> [out-prefix]
...
PREFIX="${2:-/tmp/lt-is-oracle-checks}"
...
if [ -f "${PREFIX}.java.tsv" ] && [ "${IS_REUSE_JAVA:-0}" = "1" ]; then
...
set -e
cd /lt/languagetool-language-modules/is
...
LT_LANG=is java -cp "/tmp/oracle-classes:$CP" CheckDump /in/sentences.txt > /out/checks.java.tsv
EOS
...
"$RS_ROOT/target/release/lt-cli" check -l is --lines --jobs "${IS_JOBS:-8}" \

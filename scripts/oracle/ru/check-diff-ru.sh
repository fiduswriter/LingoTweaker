#!/usr/bin/env bash
# Exact Russian match-set diff: Java LT `check()` vs `lt-cli check -l ru
# --lines`. Requires Docker for the pinned Java build.
#
# Usage: scripts/oracle/ru/check-diff-ru.sh <sentences.txt> [out-prefix]
...
PREFIX="${2:-/tmp/lt-ru-oracle-checks}"
...
if [ -f "${PREFIX}.java.tsv" ] && [ "${RU_REUSE_JAVA:-0}" = "1" ]; then
...
set -e
cd /lt/languagetool-language-modules/ru
...
LT_LANG=ru java -cp "/tmp/oracle-classes:$CP" CheckDump /in/sentences.txt > /out/checks.java.tsv
EOS
...
"$RS_ROOT/target/release/lt-cli" check -l ru --lines --jobs "${RU_JOBS:-8}" \

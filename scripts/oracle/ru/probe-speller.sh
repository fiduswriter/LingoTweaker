#!/usr/bin/env bash
# Java probe for the Russian Morfologik speller (`isMisspelled` and `match()`
# per word), for diffing against the Rust speller.
#
# Usage: scripts/oracle/ru/probe-speller.sh word1 word2 ...
set -euo pipefail
...
mkdir -p /tmp/classes
javac -cp "$CP" -d /tmp/classes /oracle-src/ru/SpellerProbe.java

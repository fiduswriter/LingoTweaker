#!/usr/bin/env bash
# Exact Polish match-set diff: Java LT `check()` vs `lt-cli check -l pl
# --lines`. Requires Docker for the pinned Java build. The Java side runs the
# default variant (pl).
#
# Usage: scripts/oracle/pl/check-diff-pl.sh <sentences.txt> [out-prefix]
...
PREFIX="${2:-/tmp/lt-pl-oracle-checks}"
...
if [ -f "${PREFIX}.java.tsv" ] && [ "${PL_REUSE_JAVA:-0}" = "1" ]; then
...
LT_LANG=pl java -cp "/tmp/oracle-classes:$CP" CheckDump /in/sentences.txt > /out/checks.java.tsv
EOS
...
"$RS_ROOT/target/release/lt-cli" check -l pl --lines --jobs "${PL_JOBS:-8}" \

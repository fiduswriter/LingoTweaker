#!/usr/bin/env bash
# Java probe for one Polish text with selected rule ids (enables default-off
# and picky rules). Usage: scripts/oracle/pl/probe-rule.sh "<text>" [ruleId ...]
set -euo pipefail
...
mkdir -p /tmp/classes
javac -cp "$CP" -d /tmp/classes /oracle-src/pl/ProbeRule.java

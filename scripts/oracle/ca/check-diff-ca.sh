#!/usr/bin/env bash
# Exact Catalan match-set diff: Java LT `check()` vs `lt-cli check -l ca
# --lines`. Requires Docker for the pinned Java build. The Java side runs the
# default variant (ca-ES); the corpus also carries ca-ES-balear/ca-ES-valencia
# rule examples, whose variant rules are inactive in Java and Rust alike.
#
# Usage: scripts/oracle/ca/check-diff-ca.sh <sentences.txt> [out-prefix]
# Writes <prefix>.java.tsv / <prefix>.rust.tsv and prints a difference summary.
# Set CA_REUSE_JAVA=1 to reuse an existing <prefix>.java.tsv (Rust-only re-run).
set -euo pipefail
...
PREFIX="${2:-/tmp/lt-ca-oracle-checks}"
...
if [ -f "${PREFIX}.java.tsv" ] && [ "${CA_REUSE_JAVA:-0}" = "1" ]; then
...
LT_LANG=ca-ES java -cp "/tmp/oracle-classes:$CP" CheckDump /in/sentences.txt > /out/checks.java.tsv
EOS
...
"$RS_ROOT/target/release/lt-cli" check -l ca --lines --jobs "${CA_JOBS:-8}" \

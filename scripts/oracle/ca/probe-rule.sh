#!/usr/bin/env bash
# Java probe for individual Catalan rules / sentences against the pinned
# LT build in Docker. Prints one TSV line per match (UTF-16 offsets, Java
# message, suggested replacements).
#
# Usage: scripts/oracle/ca/probe-rule.sh <ca-ES|ca-ES-balear|ca-ES-valencia> "<text>" [RULE_ID ...]
set -euo pipefail
...
mkdir -p /tmp/classes
javac -cp "$CP" -d /tmp/classes /oracle-src/ca/ProbeRule.java
java -cp "/tmp/classes:$CP" ProbeRule "$@" 2>/dev/null
' sh "$VARIANT" "$TEXT" "${RULES[@]}"

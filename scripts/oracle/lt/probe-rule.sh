#!/usr/bin/env bash
# Java probe for individual Lithuanian rules / sentences against the pinned
# LT build in Docker. Prints one TSV line per match (UTF-16 offsets, Java
# message, suggested replacements).
#
# Usage: scripts/oracle/lt/probe-rule.sh "<text>" [RULE_ID ...]
set -euo pipefail
...
docker run --rm \
  ...
  -w /lt "$IMAGE" sh -c '
set -e
cd /lt/languagetool-language-modules/lt
...
mkdir -p /tmp/classes
javac -cp "$CP" -d /tmp/classes /oracle-src/lt/ProbeRule.java

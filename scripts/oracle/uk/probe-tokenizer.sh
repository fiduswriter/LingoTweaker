#!/usr/bin/env bash
# Java probe for the Ukrainian word tokenizer (one token per line) against the
# pinned LT build in Docker, for diffing against the Rust tokenizer.
#
# Usage: scripts/oracle/uk/probe-tokenizer.sh "<text>"
set -euo pipefail
...
mkdir -p /tmp/classes
javac -cp "$CP" -d /tmp/classes /oracle-src/uk/TokenizerProbe.java
java -cp "/tmp/classes:$CP" TokenizerProbe "$@" 2>/dev/null
' sh "$@"

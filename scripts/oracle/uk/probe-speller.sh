#!/usr/bin/env bash
# Java probe for the Ukrainian Morfologik speller (`isMisspelled`,
# `getSuggestions` and `match()` per word), for diffing against the Rust
# speller.
#
# Usage: scripts/oracle/uk/probe-speller.sh word1 word2 ...
set -euo pipefail
...
mkdir -p /tmp/classes
javac -cp "$CP" -d /tmp/classes /oracle-src/uk/SpellerProbe.java

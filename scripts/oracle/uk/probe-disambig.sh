#!/usr/bin/env bash
# Java probe for the Ukrainian disambiguated analysis against the
# pinned LT build in Docker, for diffing against the Rust tokenizer.
#
# Usage: scripts/oracle/uk/probe-disambig.sh "<text>"
set -euo pipefail
...
mkdir -p /tmp/classes
javac -cp "$CP" -d /tmp/classes /oracle-src/uk/DisambigProbe.java
java -cp "/tmp/classes:$CP" DisambigProbe "$@" 2>/dev/null
' sh "$@"

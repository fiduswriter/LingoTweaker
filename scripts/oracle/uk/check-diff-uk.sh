#!/usr/bin/env bash
# Exact Ukrainian match-set diff: Java LT `check()` vs `lt-cli check -l uk
...
PREFIX="${2:-/tmp/lt-uk-oracle-checks}"
...
if [ -f "${PREFIX}.java.tsv" ] && [ "${UK_REUSE_JAVA:-0}" = "1" ]; then
...
LT_LANG=uk java -cp "/tmp/oracle-classes:$CP" CheckDump /in/sentences.txt > /out/checks.java.tsv
EOS
...
"$RS_ROOT/target/release/lt-cli" check -l uk --lines --jobs "${UK_JOBS:-8}" \

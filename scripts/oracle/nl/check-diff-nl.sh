#!/usr/bin/env bash
# Exact Dutch match-set diff: Java LT `check()` vs `lt-cli check -l nl
# --lines`. Requires Docker for the pinned Java build. The Java side runs the
# default variant (nl-NL); the corpus also carries nl-BE rule
# examples, whose rules are inactive in Java and Rust alike.
#
# Usage: scripts/oracle/nl/check-diff-nl.sh <sentences.txt> [out-prefix]
...
PREFIX="${2:-/tmp/lt-nl-oracle-checks}"
...
if [ -f "${PREFIX}.java.tsv" ] && [ "${NL_REUSE_JAVA:-0}" = "1" ]; then
...
LT_LANG=nl-NL java -cp "/tmp/oracle-classes:$CP" CheckDump /in/sentences.txt > /out/checks.java.tsv
EOS
...
"$RS_ROOT/target/release/lt-cli" check -l nl --lines --jobs "${NL_JOBS:-8}" \

#!/usr/bin/env bash
# Java probe for individual Dutch rules / sentences against the pinned
# LT build in Docker. Prints one TSV line per match (UTF-16 offsets, Java
# message, suggested replacements).
#
# Usage: scripts/oracle/nl/probe-rule.sh <nl-NL|nl-BE> "<text>" [RULE_ID ...]
set -euo pipefail
...
mkdir -p /tmp/classes
javac -cp "$CP" -d /tmp/classes /oracle-src/nl/ProbeRule.java

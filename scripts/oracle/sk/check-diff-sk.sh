#!/usr/bin/env bash
# Exact Slovak match-set diff: Java LT `check()` vs `lt-cli check -l sk
# --lines`. Requires Docker for the pinned Java build. The Java side runs the
# default variant (sk).
#
# Usage: scripts/oracle/sk/check-diff-sk.sh <sentences.txt> [out-prefix]
...
PREFIX="${2:-/tmp/lt-sk-oracle-checks}"
...
if [ -f "${PREFIX}.java.tsv" ] && [ "${SK_REUSE_JAVA:-0}" = "1" ]; then
...
set -e
cd /lt/languagetool-language-modules/sk
...
LT_LANG=sk java -cp "/tmp/oracle-classes:$CP" CheckDump /in/sentences.txt > /out/checks.java.tsv
EOS
...
"$RS_ROOT/target/release/lt-cli" check -l sk --lines --jobs "${SK_JOBS:-8}" \

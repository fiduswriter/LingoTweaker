#!/usr/bin/env bash
# Exact Greek match-set diff: Java LT `check()` vs `lt-cli check -l el
# --lines`. Requires Docker for the pinned Java build. The Java side runs the
# default variant (el).
#
# Usage: scripts/oracle/el/check-diff-el.sh <sentences.txt> [out-prefix]
...
PREFIX="${2:-/tmp/lt-el-oracle-checks}"
...
if [ -f "${PREFIX}.java.tsv" ] && [ "${EL_REUSE_JAVA:-0}" = "1" ]; then
...
LT_LANG=el java -cp "/tmp/oracle-classes:$CP" CheckDump /in/sentences.txt > /out/checks.java.tsv
EOS
...
"$RS_ROOT/target/release/lt-cli" check -l el --lines --jobs "${EL_JOBS:-8}" \

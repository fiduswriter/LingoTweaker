#!/usr/bin/env bash
# Exact Tagalog match-set diff: Java LT `check()` vs `lt-cli check -l tl
# --lines`. Requires Docker for the pinned Java build. The Java side runs the
# default variant (is).
#
# Usage: scripts/oracle/tl/check-diff-tl.sh <sentences.txt> [out-prefix]
...
PREFIX="${2:-/tmp/lt-tl-oracle-checks}"
...
set -e
cd /lt/languagetool-language-modules/tl
...
LT_LANG=tl java -cp "/tmp/oracle-classes:$CP" CheckDump /in/sentences.txt > /out/checks.java.tsv
EOS
...
"$RS_ROOT/target/release/lt-cli" check -l tl --lines --jobs "${TL_JOBS:-8}" \

#!/usr/bin/env bash
# Java probe for individual Tagalog rules / sentences against the pinned
# LT build in Docker. Prints one TSV line per match (UTF-16 offsets, Java
# message, suggested replacements).
#
# Usage: scripts/oracle/tl/probe-rule.sh "<text>" [RULE_ID ...]
set -euo pipefail
...
mkdir -p /tmp/classes
javac -cp "$CP" -d /tmp/classes /oracle-src/tl/ProbeRule.java

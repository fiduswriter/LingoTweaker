#!/usr/bin/env bash
# Exact Spanish match-set diff: Java LT `check()` vs `lt-cli check -l es-ES
# --lines`. Requires Docker for the pinned Java build.
#
# Usage: scripts/oracle/es/check-diff-es.sh <sentences.txt> [out-prefix]
...
PREFIX="${2:-/tmp/lt-es-oracle-checks}"
...
if [ -f "${PREFIX}.java.tsv" ] && [ "${ES_REUSE_JAVA:-0}" = "1" ]; then
...
LT_LANG=es java -cp "/tmp/oracle-classes:$CP" CheckDump /in/sentences.txt > /out/checks.java.tsv
EOS
...
"$RS_ROOT/target/release/lt-cli" check -l es --lines --jobs "${ES_JOBS:-8}" \

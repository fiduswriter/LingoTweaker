#!/usr/bin/env bash
# Exact Malayalam match-set diff: Java LT `check()` vs `lt-cli check -l ml
# --lines`. Requires Docker for the pinned Java build.
#
# Usage: scripts/oracle/ml/check-diff-ml.sh <sentences.txt> [out-prefix]
...
PREFIX="${2:-/tmp/lt-ml-oracle-checks}"
...
if [ -f "${PREFIX}.java.tsv" ] && [ "${ML_REUSE_JAVA:-0}" = "1" ]; then
...
set -e
cd /lt/languagetool-language-modules/ml
...
LT_LANG=ml java -cp "/tmp/oracle-classes:$CP" CheckDump /in/sentences.txt > /out/checks.java.tsv
EOS
...
"$RS_ROOT/target/release/lt-cli" check -l ml --lines --jobs "${ML_JOBS:-8}" \

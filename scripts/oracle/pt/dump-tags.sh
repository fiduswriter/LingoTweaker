#!/usr/bin/env bash
# Java probe for the Portuguese tokenizer + tagger readings. One input
# sentence per line; prints the same S/T lines as
# `cargo run -p lt-tagger --example dump_tags_pt`.
#
...
mkdir -p /tmp/classes
javac -cp "$CP" -d /tmp/classes /oracle-src/pt/DumpTags.java

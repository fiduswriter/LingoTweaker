#!/usr/bin/env bash
# Exact Belarusian match-set diff: Java LT `check()` vs `lt-cli check -l be
# --lines`. Requires Docker for the pinned Java build. The Java side runs the
# default variant (is).
#
# Usage: scripts/oracle/be/check-diff-be.sh <sentences.txt> [out-prefix]
...
PREFIX="${2:-/tmp/lt-be-oracle-checks}"
...
set -e
cd /lt/languagetool-language-modules/be
...
LT_LANG=be java -cp "/tmp/oracle-classes:$CP" CheckDump /in/sentences.txt > /out/checks.java.tsv
EOS
...
"$RS_ROOT/target/release/lt-cli" check -l be --lines --jobs "${BE_JOBS:-8}" \

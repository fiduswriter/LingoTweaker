#!/usr/bin/env bash
# Exact Asturian match-set diff: Java LT `check()` vs `lt-cli check -l ast
# --lines`. Requires Docker for the pinned Java build. The Java side runs the
# default variant (is).
#
# Usage: scripts/oracle/ast/check-diff-ast.sh <sentences.txt> [out-prefix]
...
PREFIX="${2:-/tmp/lt-ast-oracle-checks}"
...
set -e
cd /lt/languagetool-language-modules/ast
...
LT_LANG=ast java -cp "/tmp/oracle-classes:$CP" CheckDump /in/sentences.txt > /out/checks.java.tsv
EOS
...
"$RS_ROOT/target/release/lt-cli" check -l ast --lines --jobs "${AST_JOBS:-8}" \

#!/usr/bin/env bash
# Exact Danish match-set diff: Java LT `check()` vs `lt-cli check -l da
# --lines`. Requires Docker for the pinned Java build. The Java side runs the
# default variant (da).
#
# Usage: scripts/oracle/da/check-diff-da.sh <sentences.txt> [out-prefix]
...
PREFIX="${2:-/tmp/lt-da-oracle-checks}"
...
if [ -f "${PREFIX}.java.tsv" ] && [ "${DA_REUSE_JAVA:-0}" = "1" ]; then
...
set -e
cd /lt/languagetool-language-modules/da
...
LT_LANG=da java -cp "/tmp/oracle-classes:$CP" CheckDump /in/sentences.txt > /out/checks.java.tsv
EOS
...
"$RS_ROOT/target/release/lt-cli" check -l da --lines --jobs "${DA_JOBS:-8}" \

#!/usr/bin/env bash
# Exact Esperanto match-set diff: Java LT `check()` vs `lt-cli check -l eo
# --lines`. Requires Docker for the pinned Java build. The Java side runs the
# default variant (is).
#
# Usage: scripts/oracle/eo/check-diff-eo.sh <sentences.txt> [out-prefix]
...
PREFIX="${2:-/tmp/lt-eo-oracle-checks}"
...
if [ -f "${PREFIX}.java.tsv" ] && [ "${EO_REUSE_JAVA:-0}" = "1" ]; then
...
set -e
cd /lt/languagetool-language-modules/eo
...
LT_LANG=eo java -cp "/tmp/oracle-classes:$CP" CheckDump /in/sentences.txt > /out/checks.java.tsv
EOS
...
"$RS_ROOT/target/release/lt-cli" check -l eo --lines --jobs "${EO_JOBS:-8}" \

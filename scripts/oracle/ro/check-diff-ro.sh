#!/usr/bin/env bash
# Exact Romanian match-set diff: Java LT `check()` vs `lt-cli check -l ro
# --lines`. Requires Docker for the pinned Java build. The Java side runs the
# default variant (ro).
#
# Usage: scripts/oracle/ro/check-diff-ro.sh <sentences.txt> [out-prefix]
...
PREFIX="${2:-/tmp/lt-ro-oracle-checks}"
...
if [ -f "${PREFIX}.java.tsv" ] && [ "${RO_REUSE_JAVA:-0}" = "1" ]; then
...
set -e
cd /lt/languagetool-language-modules/ro
...
LT_LANG=ro java -cp "/tmp/oracle-classes:$CP" CheckDump /in/sentences.txt > /out/checks.java.tsv
EOS
...
"$RS_ROOT/target/release/lt-cli" check -l ro --lines --jobs "${RO_JOBS:-8}" \

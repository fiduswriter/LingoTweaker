#!/usr/bin/env bash
# Exact Simple German match-set diff: Java LT `check()` vs
# `lt-cli check -l de-DE-x-simple-language --lines`. Requires Docker for the
# pinned Java build.
#
# Usage: scripts/oracle/de-x-simple/check-diff-de-x-simple.sh <sentences.txt> [out-prefix]
...
PREFIX="${2:-/tmp/lt-de-x-simple-oracle-checks}"
...
if [ -f "${PREFIX}.java.tsv" ] && [ "${DE_X_SIMPLE_REUSE_JAVA:-0}" = "1" ]; then
...
set -e
cd /lt/languagetool-language-modules/de-DE-x-simple-language
...
LT_LANG=de-DE-x-simple-language java -cp "/tmp/oracle-classes:$CP" CheckDump /in/sentences.txt > /out/checks.java.tsv
EOS
...
"$RS_ROOT/target/release/lt-cli" check -l de-DE-x-simple-language --lines \
  --jobs "${DE_X_SIMPLE_JOBS:-8}" --file "$SENTENCES" > "$WORK/out/checks.rust.tsv"

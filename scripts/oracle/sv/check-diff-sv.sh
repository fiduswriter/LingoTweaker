#!/usr/bin/env bash
# Exact Swedish match-set diff: Java LT `check()` vs `lt-cli check -l sv
# --lines`. Requires Docker for the pinned Java build. The Java side runs the
# default variant (sv).
#
# Usage: scripts/oracle/sv/check-diff-sv.sh <sentences.txt> [out-prefix]
...
PREFIX="${2:-/tmp/lt-sv-oracle-checks}"
...
if [ -f "${PREFIX}.java.tsv" ] && [ "${SV_REUSE_JAVA:-0}" = "1" ]; then
...
LT_LANG=sv java -cp "/tmp/oracle-classes:$CP" CheckDump /in/sentences.txt > /out/checks.java.tsv
EOS
...
"$RS_ROOT/target/release/lt-cli" check -l sv --lines --jobs "${SV_JOBS:-8}" \

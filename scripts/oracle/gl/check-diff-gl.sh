#!/usr/bin/env bash
# Exact Galician match-set diff: Java LT `check()` vs `lt-cli check -l gl
# --lines`. Requires Docker for the pinned Java build. The Java side runs the
# default variant (gl).
#
# Usage: scripts/oracle/gl/check-diff-gl.sh <sentences.txt> [out-prefix]
...
PREFIX="${2:-/tmp/lt-gl-oracle-checks}"
...
if [ -f "${PREFIX}.java.tsv" ] && [ "${GL_REUSE_JAVA:-0}" = "1" ]; then
...
set -e
cd /lt/languagetool-language-modules/gl
...
LT_LANG=gl java -cp "/tmp/oracle-classes:$CP" CheckDump /in/sentences.txt > /out/checks.java.tsv
EOS
...
"$RS_ROOT/target/release/lt-cli" check -l gl --lines --jobs "${GL_JOBS:-8}" \

#!/usr/bin/env bash
# Exact Arabic match-set diff: Java LT `check()` vs `lt-cli check -l ar
# --lines`. Requires Docker for the pinned Java build.
#
# Usage: scripts/oracle/ar/check-diff-ar.sh <sentences.txt> [out-prefix]
...
PREFIX="${2:-/tmp/lt-ar-oracle-checks}"
...
if [ -f "${PREFIX}.java.tsv" ] && [ "${AR_REUSE_JAVA:-0}" = "1" ]; then
...
LT_LANG=ar java -cp "/tmp/oracle-classes:$CP" CheckDump /in/sentences.txt > /out/checks.java.tsv
EOS
...
"$RS_ROOT/target/release/lt-cli" check -l ar --lines --jobs "${AR_JOBS:-8}" \

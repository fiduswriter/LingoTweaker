#!/usr/bin/env bash
# Java probe for the Arabic tagger (readings per word) against the pinned LT
# build in Docker, for diffing against the Rust tagger.
#
# Usage: scripts/oracle/ar/probe-tagger.sh word1 word2 ...
set -euo pipefail
...
mkdir -p /tmp/classes
javac -cp "$CP" -d /tmp/classes /oracle-src/ar/TaggerProbe.java

#!/usr/bin/env bash
# Exact Breton match-set diff: Java LT `check()` vs `lt-cli check -l br
# --lines`. Requires Docker for the pinned Java build. The Java side runs the
# default variant (is).
#
# Usage: scripts/oracle/br/check-diff-br.sh <sentences.txt> [out-prefix]
...
PREFIX="${2:-/tmp/lt-br-oracle-checks}"
...
set -e
cd /lt/languagetool-language-modules/br
...
LT_LANG=br java -cp "/tmp/oracle-classes:$CP" CheckDump /in/sentences.txt > /out/checks.java.tsv
EOS
...
"$RS_ROOT/target/release/lt-cli" check -l br --lines --jobs "${BR_JOBS:-8}" \

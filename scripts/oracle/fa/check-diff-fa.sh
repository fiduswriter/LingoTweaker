#!/usr/bin/env bash
# Exact Persian match-set diff: Java LT `check()` vs `lt-cli check -l fa
# --lines`. Requires Docker for the pinned Java build.
#
# Usage: scripts/oracle/fa/check-diff-fa.sh <sentences.txt> [out-prefix]
...
PREFIX="${2:-/tmp/lt-fa-oracle-checks}"
...
if [ -f "${PREFIX}.java.tsv" ] && [ "${FA_REUSE_JAVA:-0}" = "1" ]; then
...
set -e
cd /lt/languagetool-language-modules/fa
...
LT_LANG=fa java -cp "/tmp/oracle-classes:$CP" CheckDump /in/sentences.txt > /out/checks.java.tsv
EOS
...
"$RS_ROOT/target/release/lt-cli" check -l fa --lines --jobs "${FA_JOBS:-8}" \

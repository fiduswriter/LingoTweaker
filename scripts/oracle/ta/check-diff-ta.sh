#!/usr/bin/env bash
# Exact Tamil match-set diff: Java LT `check()` vs `lt-cli check -l ta
# --lines`. Requires Docker for the pinned Java build.
#
# Usage: scripts/oracle/ta/check-diff-ta.sh <sentences.txt> [out-prefix]
...
PREFIX="${2:-/tmp/lt-ta-oracle-checks}"
...
if [ -f "${PREFIX}.java.tsv" ] && [ "${TA_REUSE_JAVA:-0}" = "1" ]; then
...
set -e
cd /lt/languagetool-language-modules/ta
...
LT_LANG=ta java -cp "/tmp/oracle-classes:$CP" CheckDump /in/sentences.txt > /out/checks.java.tsv
EOS
...
"$RS_ROOT/target/release/lt-cli" check -l ta --lines --jobs "${TA_JOBS:-8}" \

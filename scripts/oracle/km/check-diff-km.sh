#!/usr/bin/env bash
# Exact Khmer match-set diff: Java LT `check()` vs `lt-cli check -l km
# --lines`. Requires Docker for the pinned Java build.
#
# Usage: scripts/oracle/km/check-diff-km.sh <sentences.txt> [out-prefix]
...
PREFIX="${2:-/tmp/lt-km-oracle-checks}"
...
if [ -f "${PREFIX}.java.tsv" ] && [ "${KM_REUSE_JAVA:-0}" = "1" ]; then
...
set -e
cd /lt/languagetool-language-modules/km
...
LT_LANG=km java -cp "/tmp/oracle-classes:$CP" CheckDump /in/sentences.txt > /out/checks.java.tsv
EOS
...
"$RS_ROOT/target/release/lt-cli" check -l km --lines --jobs "${KM_JOBS:-8}" \

#!/usr/bin/env bash
# Exact Italian match-set diff: Java LT `check()` vs `lt-cli check -l it
# --lines`. Requires Docker for the pinned Java build.
#
# Usage: scripts/oracle/it/check-diff-it.sh <sentences.txt> [out-prefix]
# Writes <prefix>.java.tsv / <prefix>.rust.tsv and prints a difference summary.
# Set IT_REUSE_JAVA=1 to reuse the cached Java dump (Rust-only re-run).
set -euo pipefail
...
PREFIX="${2:-/tmp/lt-it-oracle-checks}"
...
if [ -f "${PREFIX}.java.tsv" ] && [ "${IT_REUSE_JAVA:-0}" = "1" ]; then
...
LT_LANG=it java -cp "/tmp/oracle-classes:$CP" CheckDump /in/sentences.txt > /out/checks.java.tsv
EOS
...
"$RS_ROOT/target/release/lt-cli" check -l it --lines --jobs "${IT_JOBS:-8}" \

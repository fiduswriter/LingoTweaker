#!/usr/bin/env bash
# Java probe for the Italian tokenizer + tagger readings. One input sentence
# per line; prints the same S/T lines as `cargo run -p lt-tagger --example
# dump_tags_it`.
#
# Usage: scripts/oracle/it/dump-tags.sh sentences.txt
set -euo pipefail
...
mkdir -p /tmp/classes
javac -cp "$CP" -d /tmp/classes /oracle-src/it/DumpTags.java

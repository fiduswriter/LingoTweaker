#!/usr/bin/env bash
# Java probe for the German post-disambiguation pipeline incl. GermanChunker
# chunk tags. One input sentence per line; prints D lines that `lt-cli analyze
# -l de-DE` produces as well.
#
# Usage: scripts/oracle/de/probe-chunk.sh sentences.txt [de-DE|de-AT|de-CH]
set -euo pipefail
...
mkdir -p /tmp/classes
javac -cp "$CP" -d /tmp/classes /oracle-src/de/ProbeChunk.java
java -cp "/tmp/classes:$CP" ProbeChunk "/input/'"$(basename "$FILE")"'" "'"$VARIANT"'"
' 2>/dev/null

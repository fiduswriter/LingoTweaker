#!/usr/bin/env bash
# Java probe for the German speller: one word per line, prints
# `word<TAB>isMisspelled<TAB>suggestion|suggestion|...` (Java order).
#
# Usage: scripts/oracle/de/probe-speller.sh words.txt [de-DE|de-AT|de-CH]
set -euo pipefail
...
mkdir -p /tmp/classes
javac -cp "$CP" -d /tmp/classes /oracle-src/de/ProbeSpeller.java
java -cp "/tmp/classes:$CP" ProbeSpeller "/input/'"$(basename "$FILE")"'" "'"$VARIANT"'"
' 2>/dev/null

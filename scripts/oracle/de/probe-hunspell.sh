#!/usr/bin/env bash
# Java probe for raw native-hunspell `spell()` acceptance: one word per line,
# prints `word<TAB>spell()` using the same DumontsHunspellDictionary the
# German speller rule uses (no LT ignore lists).
#
# Usage: scripts/oracle/de/probe-hunspell.sh words.txt [de_DE|de_AT|de_CH]
set -euo pipefail
...
mkdir -p /tmp/classes
javac -cp "$CP" -d /tmp/classes /oracle-src/de/ProbeHunspell.java
java -cp "/tmp/classes:$CP" ProbeHunspell /lt/languagetool-language-modules/de/src/main/resources/org/languagetool/resource/de/hunspell "$VARIANT" "/input/$INPUT_NAME"
' 2>/dev/null

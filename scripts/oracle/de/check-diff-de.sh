#!/usr/bin/env bash
# Exact German match-set diff: Java LT `check()` vs `lt-cli check -l de-DE
# --lines`. Requires Docker for the pinned Java build.
#
# Usage: scripts/oracle/de/check-diff-de.sh <sentences.txt> [out-prefix]
...
PREFIX="${2:-/tmp/lt-de-oracle-checks}"
...
if [ -f "${PREFIX}.java.tsv" ] && [ "${DE_REUSE_JAVA:-0}" = "1" ]; then
...
LT_LANG=de-DE java -cp "/tmp/oracle-classes:$CP" CheckDump /in/sentences.txt > /out/checks.java.tsv
EOS
...
"$RS_ROOT/target/release/lt-cli" check -l de-DE --lines --jobs "${DE_JOBS:-8}" \

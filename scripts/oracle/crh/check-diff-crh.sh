#!/usr/bin/env bash
# Exact Crimean Tatar match-set diff: Java LT `check()` vs `lt-cli check -l crh
# --lines`. Requires Docker for the pinned Java build. The Java side runs the
# default variant (is).
#
# Usage: scripts/oracle/crh/check-diff-crh.sh <sentences.txt> [out-prefix]
...
PREFIX="${2:-/tmp/lt-crh-oracle-checks}"
...
if [ -f "${PREFIX}.java.tsv" ] && [ "${AST_REUSE_JAVA:-0}" = "1" ]; then
...
set -e
cd /lt/languagetool-language-modules/crh
...
LT_LANG=crh java -cp "/tmp/oracle-classes:$CP" CheckDump /in/sentences.txt > /out/checks.java.tsv
EOS
...
"$RS_ROOT/target/release/lt-cli" check -l crh --lines --jobs "${CRH_JOBS:-8}" \

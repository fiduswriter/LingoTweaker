#!/usr/bin/env bash
# Java probe for individual French rules against the pinned LT build in
# Docker. Prints one TSV line per match of the requested rules (UTF-16
# offsets, Java message, suggested replacements).
#
# Usage: scripts/oracle/fr/probe-rule.sh "<text>" [RULE_ID ...]
set -euo pipefail
...
mkdir -p /tmp/classes
javac -cp "$CP" -d /tmp/classes /oracle-src/fr/ProbeRule.java

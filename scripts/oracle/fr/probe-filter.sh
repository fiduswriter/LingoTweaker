#!/usr/bin/env bash
# Java probe for the real FindSuggestionsFilter.getSpellingSuggestions.
#
# Usage: scripts/oracle/fr/probe-filter.sh <surface> <postag>
set -euo pipefail
...
mkdir -p /tmp/classes
javac -cp "$CP" -d /tmp/classes /oracle-src/fr/FilterProbe.java
java -cp "/tmp/classes:$CP" FilterProbe "$@" 2>/dev/null
' sh "$@"

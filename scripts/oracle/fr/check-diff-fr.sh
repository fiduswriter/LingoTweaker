#!/usr/bin/env bash
# Exact French match-set diff: Java LT `check()` vs `lt-cli check -l fr
# --lines`. Requires Docker for the pinned Java build.
#
# Usage: scripts/oracle/fr/check-diff-fr.sh <sentences.txt> [out-prefix]
...
PREFIX="${2:-/tmp/lt-fr-oracle-checks}"
...
if [ -f "${PREFIX}.java.tsv" ] && [ "${FR_REUSE_JAVA:-0}" = "1" ]; then
...
LT_LANG=fr java -cp "/tmp/oracle-classes:$CP" CheckDump /in/sentences.txt > /out/checks.java.tsv
EOS
...
"$RS_ROOT/target/release/lt-cli" check -l fr --lines --jobs "${FR_JOBS:-8}" \

#!/usr/bin/env bash
# Java probe for the Soros number speller used by French `_spell_number_`
# rules. Prints `input TAB spelled` per argument (Docker, pinned LT build).
#
# Usage: scripts/oracle/fr/probe-soros.sh <numeral> ...
set -euo pipefail
...
mkdir -p /tmp/classes
javac -cp "$CP" -d /tmp/classes /oracle-src/fr/SorosProbe.java
java -cp "/tmp/classes:$CP" SorosProbe "$@" 2>/dev/null
' sh "$@"

#!/usr/bin/env bash
# Java probe for FRENCH_WORD_REPEAT_RULE match internals (token view +
# pattern positions) against the pinned LT build.
#
# Usage: scripts/oracle/fr/probe-repeat.sh "<text>"
set -euo pipefail
...
mkdir -p /tmp/classes
javac -cp "$CP" -d /tmp/classes /oracle-src/fr/RepeatProbe.java
java -cp "/tmp/classes:$CP" RepeatProbe "$@" 2>/dev/null
' sh "$@"

#!/usr/bin/env bash
# Java probe for FrenchSynthesizer.synthesize (lemma|postag|regexp args).
#
# Usage: scripts/oracle/fr/probe-synth.sh "exister|V ppa m s|false" ...
set -euo pipefail
...
mkdir -p /tmp/classes
javac -cp "$CP" -d /tmp/classes /oracle-src/fr/SynthProbe.java

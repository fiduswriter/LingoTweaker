#!/usr/bin/env bash
# Java probe for the French post-disambiguation token stream. One input
# sentence per line; prints D lines that `lt-cli analyze -l es` produces as
# well (UTF-16 `startPos` in Java, UTF-8 bytes in Rust).
#
# Usage: scripts/oracle/fr/probe-disambig.sh sentences.txt
set -euo pipefail
...
mkdir -p /tmp/classes
javac -cp "$CP" -d /tmp/classes /oracle-src/fr/ProbeDisambig.java

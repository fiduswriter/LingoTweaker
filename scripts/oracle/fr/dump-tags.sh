#!/usr/bin/env bash
# Java probe for the French tokenizer + tagger readings. One input sentence
# per line; prints the same S/T lines as `cargo run -p lt-tagger --example
# dump_tags_es`.
#
# Usage: scripts/oracle/fr/dump-tags.sh sentences.txt
set -euo pipefail
...
mkdir -p /tmp/classes
javac -cp "$CP" -d /tmp/classes /oracle-src/fr/DumpTags.java

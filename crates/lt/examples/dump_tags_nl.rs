//! Dumps raw Dutch tagger readings in the same TSV format as
//! `scripts/oracle/nl/DumpTags.java` (tagger verification).
//!
//! Usage: cargo run -p lt --example dump_tags_nl -- <sentences.txt>
...
fn main() {
    ...
    let path = PathBuf::from(args.next().expect("usage: dump_tags_nl <sentences.txt>"));

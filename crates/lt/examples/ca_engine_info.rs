//! Stage-1/3 Catalan engine report: active XML rules, disambiguation rules,
//! skipped counts and compile failures per variant.
//!
//! Usage: `cargo run -p lt --example ca_engine_info [ca-ES|ca-ES-balear|ca-ES-valencia]`

fn main() {
    let variant = std::env::args()
        .nth(1)
        .unwrap_or_else(|| "ca-ES".to_string());
    let data = lt::DataDir::discover().unwrap();
    let engine = lt::Engine::builder(lt::Lang::Ca)
        .unwrap()
        .data_dir(data)

//! Dumps raw Catalan tagger readings in the same TSV format as
//! `scripts/oracle/ca/DumpTags.java` (tagger verification).
//!
//! Usage: cargo run -p lt --example dump_tags_ca -- <sentences.txt>
...
fn main() {
    ...
    let path = PathBuf::from(args.next().expect("usage: dump_tags_ca <sentences.txt>"));
    let data = lt::DataDir::discover().expect("data dir");

//! German date filters (`DateFilterHelper`, `DateCheckFilter`,
//! `FutureDateFilter`, `NewYearDateFilter`, `YMDNewYearDateFilter`,
//! `YMDDateCheckFilter`) — the German subclasses of the core abstract
//! filters, ported from the same logic as the English instances in
//! `crate::en::filters` with the German day/month names, the German
//! wrong-year message, `DateCheckFilter.adjustSuggestion` and
//! `Locale.GERMANY` today.
...
/// `DateFilterHelper.getDayOfWeek(String)` (German prefixes; Java
...
/// `DateFilterHelper.getMonth` (German prefixes, 1-based).
...
/// `DateFilterHelper.getDayOfWeek(Calendar)` (`Locale.GERMAN`, LONG).
...
/// `DateCheckFilter.adjustSuggestion`.
fn adjust_suggestion(sugg: String) -> String {
    let dot_comma_pos = sugg.find(".,");
    if let Some(pos) = dot_comma_pos {
        if pos > 5 && pos < 12 {
            return sugg.replace(".,", ",");
        }
    }
    let comma_pos = sugg.find(',');
    if dot_comma_pos.is_none() {
        if let Some(pos) = comma_pos {
            if pos > 0 && pos < 5 {
                return sugg.replacen(',', ".,", 1);
            }
        }
    }
    sugg
...
impl RuleFilter for DateCheckFilter {
    fn accept(&self, ctx: &FilterContext) -> FilterOutcome {
        ...
        if dates::weekday(today.year, month, day) == Some(day_of_week_from_string) {
            let message = format!(
                "Dieses Datum stimmt nicht mit dem Tag überein. Meinten Sie \"{}\"?",
                today.year
            );

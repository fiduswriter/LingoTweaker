//! Guaraní `getPriorityForId` for `CleanOverlappingFilter`.
//!
...
pub fn priority_for_id(id: &str) -> i32 {
    ...
    if id == "GN_ACCENTS" {
        // Generic dictionary-driven fallback: curated list rules win.
        return -1;
    }
    if id == "GN_HARMONY" {
        // Dictionary-driven nasal harmony: curated list rules win, the
        // rule wins over `GN_SPELLER`.
        return -1;
    }

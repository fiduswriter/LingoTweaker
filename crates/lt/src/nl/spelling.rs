//! Dutch spelling rule (`MorfologikDutchSpellerRule`): port of the
//! morfologik speller path of `MorfologikSpellerRule` and
//! `SpellingCheckRule` plus the Dutch overrides.
//!
//! Dutch specifics: the binary dictionary is `nl/spelling/nl_NL.dict`
//! (CFSA2, UTF-8) with the `nl_NL.info` speller options, the
//! ignore/spelling/prohibit lists live in `nl/spelling/`,
//! `getRuleMatches` skips `_english_ignore_` tokens, and
//! `ignorePotentiallyMisspelledWord` delegates to `CompoundAcceptor`
//! (set after construction; the acceptor calls back into `spellingOk`).
...
pub const RULE_ID: &str = "MORFOLOGIK_RULE_NL_NL";
const DESCRIPTION: &str = "Waarschijnlijk een spelfout";
const MESSAGE: &str = "Er is een mogelijke spelfout gevonden.";
const SHORT_MESSAGE: &str = "Spelfout";
const CATEGORY_ID: &str = "TYPOS";
const CATEGORY_NAME: &str = "Mogelijke typefouten";
...
pub struct DutchSpellingRule {
    /// the binary `nl_NL.dict` alone (`MorfologikSpeller` for
    ...
    /// `MorfologikDutchSpellerRule` does not call `setIgnoreTaggedWords()`
    ignore_tagged_words: bool,
    /// `Dutch.getCompoundAcceptor()` for `ignorePotentiallyMisspelledWord`;
    /// set by the pipeline after both sides exist.
    compound_acceptor: std::sync::OnceLock<Arc<crate::nl::compound_acceptor::CompoundAcceptor>>,
    /// IGNORE_SPELLING anti-pattern phrases from multi-word ignore entries
    /// (`SpellingCheckRule.addIgnoreWords` with `tokenizeNewWords() == true`),
    /// keyed by their first token.
    ignored_phrases: std::collections::HashMap<String, Vec<Vec<String>>>,
    suggestion_cache:
        std::sync::Mutex<std::collections::HashMap<String, std::sync::Arc<Vec<Suggestion>>>>,
}

impl DutchSpellingRule {
    pub fn load(data_dir: &Path) -> Result<Self> {
        let spelling_dir = data_dir.join("nl/spelling");
        let info = DictionaryInfo::load(&spelling_dir.join("nl_NL.info"))?;
        let meta = SpellerMetadata::from_info(&info)?;
        let binary_source = Arc::new(DictSource::from_dict_file(
            &spelling_dir.join("nl_NL.dict"),
            &spelling_dir.join("nl_NL.info"),
        )?);
        ...
        let mut rule = Self {
            ...
            compound_acceptor: std::sync::OnceLock::new(),
            ignored_phrases: std::collections::HashMap::new(),
        ...
        for path in [
            spelling_dir.join("ignore.txt"),
            spelling_dir.join("spelling.txt"),
            data_dir.join("nl/hunspell/spelling_custom.txt"),
            data_dir.join("core/spelling_global.txt"),
        ] {
            rule.load_ignore(&path);
        }
        for path in [
            spelling_dir.join("prohibit.txt"),
            data_dir.join("nl/hunspell/prohibit_custom.txt"),
        ] {
    ...
    pub fn set_compound_acceptor(
        &self,
        acceptor: Arc<crate::nl::compound_acceptor::CompoundAcceptor>,
    ) {
    ...
    fn ignore_potentially_misspelled_word(&self, word: &str) -> bool {
        self.compound_acceptor
            .get()
            .is_some_and(|acceptor| acceptor.accept_compound(word))
    }
    ...
    fn load_ignore(&mut self, path: &Path) {
        for word in cache_word_list(path) {
            // `tokenizeNewWords()` is true for Dutch: multi-token entries
            // become IGNORE_SPELLING anti-patterns instead of ignore words.
            let tokens: Vec<String> = lt_tokenize::DutchWordTokenizer
                .tokenize(&word)
                .into_iter()
    ...
    pub fn check_sentence(
        ...
        let non_blank: Vec<&AnalyzedTokenReadings> = tokens
            ...
            .collect();
        // `Rule.getSentenceWithImmunization`: the IGNORE_SPELLING
        // anti-patterns (multi-token ignore entries) mark every matched
        // token as ignored by the speller.
        let mut ignored_by_phrase = vec![false; non_blank.len()];
        for (start, token) in non_blank.iter().enumerate() {
            let Some(phrases) = self.ignored_phrases.get(token.surface()) else {
                continue;
            };
            for phrase in phrases {
                let end = start + phrase.len();
                if end <= non_blank.len()
                    && phrase
                        .iter()
                        .zip(&non_blank[start..end])
                        .all(|(part, atr)| atr.surface() == part)
                {
                    for flag in ignored_by_phrase[start..end].iter_mut() {
                        *flag = true;
                    }
                }
            }
    ...
    /// `MorfologikDutchSpellerRule.getRuleMatches`: the base matches with
    /// the `_english_ignore_` check.
    fn get_rule_matches(
        &self,
        word: &str,
        start_pos: usize,
    ...
    /// `MorfologikSpellerRule.calcSpellerSuggestions` (no Dutch top or
    ...
    fn calc_speller_suggestions(&self, word: &str) -> Vec<Suggestion> {
        ...
        // sentinel suggestions (Dutch does not override this).
    ...
    /// `MorfologikSpellerRule.orderSuggestions` (base: unchanged).
...
/// `Language.prepareLineForSpeller` is the identity for Dutch. Java adds

//! Tamil (`ta`) pipeline parts. `Tamil` is a plain `Language` with a
//! `BaseTagger` (`TamilTagger`) over the in-tree FSA5 `tamil.dict`, the base
//! `WordTokenizer` (Tamil is written with spaces) and the base no-op
//! disambiguator (there is no `ta/disambiguation.xml`). There is no
//! synthesizer and no speller (`createDefaultSpellingRule` is the base
//! `null`).
//!
//! `Tamil.getRelevantRules` returns exactly five classes and does **not** call
//! `super`: the generic `CommaWhitespaceRule`, `DoublePunctuationRule`,
//! `MultipleWhitespaceRule`, `LongSentenceRule(messages, userConfig, 50)` and
//! `SentenceWhitespaceRule`. There are no ta-specific rule classes and no XML
//! `<filter>`.
...
/// `Tamil` pipeline parts.
pub struct TamilPipeline {
    /// `TamilTagger` (`BaseTagger` over `ta/dictionaries/tamil.dict`).
    pub tagger: Arc<lt_tagger::TamilTagger>,
}

/// Tokenize + tag one sentence like Java's `Tamil`: the base `WordTokenizer`
/// (`joinEMailsAndUrls` + the base tokenizing-character set) followed by the
/// `TamilTagger` (`BaseTagger`). The token stream concatenates back to the
/// original text exactly and the byte offsets accumulate.
pub fn analyze_tamil_sentence(ta: &TamilPipeline, text: &str) -> AnalyzedSentence {
    ...
    let tagged = ta.tagger.tag(&raw_tokens);

//! Asturian pipeline parts: the `AsturianTagger` (`BaseTagger` over
//! `ast/dictionaries/asturian.dict` + the manual word lists), the no-op
//! `DemoDisambiguator` order and the `MorfologikAsturianSpellerRule`.
//! `Asturian.getRelevantRules` has no Java rule classes (only the generic
//! built-ins); Asturian has no synthesizer.
...
/// `Asturian` pipeline parts. `Asturian` does not override
/// `createDefaultDisambiguator`, so the base no-op `DemoDisambiguator`
/// applies.
pub struct AsturianPipeline {
    pub tagger: Arc<lt_tagger::AsturianTagger>,
    /// `MorfologikAsturianSpellerRule` (`MORFOLOGIK_RULE_AST`). `None` only
    /// when the vendored `ast_ES` dictionary cannot be read.
    pub spelling: Option<Arc<crate::ast::spelling::AsturianSpellingRule>>,
}

impl AsturianPipeline {
    pub fn disambiguate(&self, _sentence: &mut AnalyzedSentence) {}
}

/// Asturian sentence tokenization (base `WordTokenizer`) + tagger.
pub fn analyze_asturian_sentence(asturian: &AsturianPipeline, text: &str) -> AnalyzedSentence {
    ...
    let tagged = asturian.tagger.tag(&raw_tokens);

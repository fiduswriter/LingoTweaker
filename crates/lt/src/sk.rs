//! Slovak pipeline parts: the `SlovakTagger`/`SlovakSynthesizer` over the
//! vendored Morfologik dictionaries, the plain `XmlRuleDisambiguator` order
//! and (incrementally) the Java-coded Slovak built-in rules.
//!
//! Stage 1 (XML rules) wires the tagger/synthesizer and the XML
//! disambiguator. The Morfologik speller and the Java rule classes follow in
//! stages 2/3.
...
/// `Slovak.createDefaultDisambiguator` is the base plain
/// `XmlRuleDisambiguator` (no chunker step).
pub struct SlovakPipeline {
    pub tagger: Arc<lt_tagger::SlovakTagger>,
    pub synthesizer: Arc<lt_tagger::SlovakSynthesizer>,
    /// The same synthesizer through the pattern engine's trait.
    pub synth_adapter: Arc<SlovakSynthesizerAdapter>,
    pub disambiguator: lt_disambig::XmlDisambiguator,
    /// `MorfologikSlovakSpellerRule` (`MORFOLOGIK_RULE_SK_SK`).
    pub spelling: Option<Arc<crate::sk::spelling::SlovakSpellingRule>>,
    /// `WordRepeatRule` (`WORD_REPEAT_RULE`), the generic built-in.
    pub word_repeat: crate::word_repeat::WordRepeatRule,
    /// `CompoundRule` (`SK_COMPOUNDS`, rule 8).
...
/// Adapter exposing the Slovak synthesizer through the pattern engine's
/// [`Synthesizer`] trait, plus the tagger for the `checksSpelling` check.
pub struct SlovakSynthesizerAdapter {
    pub synth: Arc<lt_tagger::SlovakSynthesizer>,
    pub tagger: Arc<lt_tagger::SlovakTagger>,
}

impl SlovakSynthesizerAdapter {
    pub fn inner(&self) -> &lt_tagger::SlovakSynthesizer {
        &self.synth
    }
}

impl Synthesizer for SlovakSynthesizerAdapter {
    fn synthesize(
        &self,
        token: &AnalyzedToken,
        pos_tag: &str,
...
/// Slovak sentence tokenization + tagger. `SlovakTagger` is a plain
/// `BaseTagger`, and the default word tokenizer is the base `WordTokenizer`
/// (base tokenizing characters), so the token stream concatenates back to the
/// original text and the byte offsets accumulate.
pub fn analyze_slovak_sentence(slovak: &SlovakPipeline, text: &str) -> AnalyzedSentence {
    ...
    let tagged = slovak.tagger.tag(&raw_tokens);

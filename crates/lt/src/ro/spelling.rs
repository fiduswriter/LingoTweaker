//! Romanian spelling rule (`MorfologikRomanianSpellerRule`): port of the
//! morfologik speller path of `MorfologikSpellerRule` and
//! `SpellingCheckRule` with no language override beyond the file name/id.
//!
//! The binary dictionary is `ro/hunspell/ro_RO.dict` (CFSA2, UTF-8); ignore /
//! spelling / prohibit lists come from `ro/hunspell/` plus the global
...
pub const RULE_ID: &str = "MORFOLOGIK_RULE_RO_RO";
const DESCRIPTION: &str = "Posibilă greșeală de ortografie";
const MESSAGE: &str = "S-a găsit o posibilă greșeală de ortografie";
const SHORT_MESSAGE: &str = "Greșeală de ortografie";
const CATEGORY_ID: &str = "TYPOS";
const CATEGORY_NAME: &str = "Posibilă greșeală de tastare";
...
pub struct RomanianSpellingRule {
    /// the binary `ro_RO.dict` alone (`MorfologikSpeller` for
    ...
    /// `MorfologikRomanianSpellerRule` does not call `setIgnoreTaggedWords()`
    ignore_tagged_words: bool,
    suggestion_cache:
        std::sync::Mutex<std::collections::HashMap<String, std::sync::Arc<Vec<Suggestion>>>>,
}

impl RomanianSpellingRule {
    pub fn load(data_dir: &Path) -> Result<Self> {
        let hunspell = data_dir.join("ro/hunspell");
        let info = DictionaryInfo::load(&hunspell.join("ro_RO.info"))?;
        let meta = SpellerMetadata::from_info(&info)?;
        let binary_source = Arc::new(DictSource::from_dict_file(
            &hunspell.join("ro_RO.dict"),
            &hunspell.join("ro_RO.info"),
        )?);
    ...
    /// `MorfologikSpellerRule.getRuleMatches` (no Romanian override).
    fn get_rule_matches(
        &self,
        word: &str,
        start_pos: usize,
    ...
    /// `MorfologikSpellerRule.calcSpellerSuggestions` (no Romanian top or
    ...
    fn calc_speller_suggestions(&self, word: &str) -> Vec<Suggestion> {
        ...
        // sentinel suggestions (Romanian does not override this).
        ...
        // `orderSuggestions` is the base identity for Romanian.
...
/// `Language.prepareLineForSpeller` is the identity for Romanian.

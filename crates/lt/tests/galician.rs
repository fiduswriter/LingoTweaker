...
/// `HunspellRule.match` tokenizes the sentence text with the `WORDCHARS -'`
/// non-word pattern: quoted (`'a'`) and alphanumeric (`241Am` -> `Am`) tokens
...
/// table (`-`, `^-`, `-$`), so a trailing-hyphen token like `Tras-` is
/// accepted.
#[test]
fn galician_speller_default_hyphen_break() {
    let matches = one("Alfándega da Fe fica en Tras- os-Montes.", "HUNSPELL_RULE");
    assert!(matches.is_empty(), "{matches:?}");
}

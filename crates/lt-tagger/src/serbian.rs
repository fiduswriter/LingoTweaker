//! Serbian tagger: port of `org.languagetool.tagging.sr.EkavianTagger`
//! (`extends SerbianTagger extends BaseTagger`) over the ekavian
//! `serbian.dict` Morfologik dictionary with the manual `added.txt` /
//! `removed.txt` lists in `sr/dictionaries/ekavian/`.
//!
//! `SerbianTagger` has no `tag()` override and no `additionalTags`
//! heuristics, so this is the plain `BaseTagger` flow
//! (`tagLowercaseWithUppercase` defaults to `true`).
...
/// Port of the plain `BaseTagger` over the loaded Serbian dictionary.
#[derive(Debug)]
pub struct EkavianTagger {
    dict: Dictionary,
    /// `ekavian/added.txt` (`getManualAdditionsFileName`).
    manual: ManualTagger,
    /// `ekavian/removed.txt` (`getManualRemovalsFileName`).
    removals: ManualTagger,
}

impl EkavianTagger {
    /// Load `sr/dictionaries/ekavian/serbian.{dict,info}` plus the manual word
    /// lists (`SerbianTagger.getManualAdditionsFileName` /
    /// `getManualRemovalsFileName`).
    pub fn load(data_dir: &Path) -> Result<Self> {
        let dict_dir = data_dir.join("sr/dictionaries/ekavian");
        let info = DictionaryInfo::load(&dict_dir.join("serbian.info"))?;
        let dict = Dictionary::load(&dict_dir.join("serbian.dict"), &info)?;
        let manual = ManualTagger::load(&[&dict_dir.join("added.txt")])?;
        let removals = ManualTagger::load(&[&dict_dir.join("removed.txt")])?;
        Ok(Self {
            dict,
            manual,
            removals,
    ...
    /// `BaseTagger.getAnalyzedTokens` for one word (no `additionalTags`,
    /// `tagLowercaseWithUppercase = true`).
    ...
    /// `EkavianTagger.INSTANCE.tag([word]).get(0).isTagged()`.

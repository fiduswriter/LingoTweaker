//! Port of `org.languagetool.synthesis.sk.SlovakSynthesizer` (+ the parts of
//! `BaseSynthesizer` it uses): inflected forms for a lemma and POS tag.
//!
//! The Slovak synthesizer is a plain `BaseSynthesizer` — no language
//! overrides — so this is the unmodified base lookup: `slovak_synth.dict`
//! keyed by `lemma|tag`, the manual `added.txt`/`removed.txt` synthesizers
//! and the tag list from `slovak_tags.txt`.
...
impl SlovakSynthesizer {
    /// Load from the vendored data directory (`sk/dictionaries`, `sk/words`).
    pub fn from_data(data_dir: &Path) -> Result<Self> {
        let dict_dir = data_dir.join("sk/dictionaries");
        let info = DictionaryInfo::load(&dict_dir.join("slovak_synth.info"))?;
        let dict = SynthDictionary::load(&dict_dir.join("slovak_synth.dict"), &info)?;
        let tag_text = lt_data::fs::read_to_string(dict_dir.join("slovak_tags.txt"))
            .map_err(|e| CoreError::Data(format!("cannot read slovak_tags.txt: {e}")))?;
        let mut possible_tags: Vec<String> = tag_text
            ...
            .map(str::to_string)
            .collect();
        let words_dir = data_dir.join("sk/words");
    ...
    /// file name for Slovak, so `getSpelledNumber` returns the numeral
    /// unchanged (`_spell_number_:feminine` therefore prepends `"feminine "`).
    /// `Roman.sor` is only used by the `_spell_number_:Roman` tag.
    pub fn synthesize(
        &self,
        token: &AnalyzedToken,
        pos_tag: &str,

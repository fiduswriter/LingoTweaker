//! Tamil tagger: port of `org.languagetool.language.tagging.TamilTagger`, a
//! plain `BaseTagger` over the `tamil.dict` Morfologik dictionary (no
...
impl TamilTagger {
    /// Load `ta/dictionaries/tamil.{dict,info}` plus the manual word lists.
    pub fn load(data_dir: &Path) -> Result<Self> {
        let dict_dir = data_dir.join("ta/dictionaries");
        let info = DictionaryInfo::load(&dict_dir.join("tamil.info"))?;
        let dict = Dictionary::load(&dict_dir.join("tamil.dict"), &info)?;
        let words_dir = data_dir.join("ta/words");
    ...
    /// `BaseTagger.getAnalyzedTokens` for one word (Tamil: no
    ...
    /// Tokenizer helper: `TamilTagger.INSTANCE.tag([word]).get(0).isTagged()`.

//! Slovak tagger: port of `org.languagetool.tagging.sk.SlovakTagger`, a plain
//! `BaseTagger` over the `slovak.dict` Morfologik dictionary (no
//! `additionalTags` heuristics, no apostrophe rewriting).
...
/// Port of the plain `BaseTagger` over the loaded Slovak dictionary.
#[derive(Debug)]
pub struct SlovakTagger {
    dict: Dictionary,
...
impl SlovakTagger {
    /// Load `sk/dictionaries/slovak.{dict,info}` plus the manual word lists.
    pub fn load(data_dir: &Path) -> Result<Self> {
        let dict_dir = data_dir.join("sk/dictionaries");
        let info = DictionaryInfo::load(&dict_dir.join("slovak.info"))?;
        let dict = Dictionary::load(&dict_dir.join("slovak.dict"), &info)?;
    ...
    /// `BaseTagger.getAnalyzedTokens` for one word (Slovak: no
    ...
    /// Tokenizer helper: `SlovakTagger.INSTANCE.tag([word]).get(0).isTagged()`.

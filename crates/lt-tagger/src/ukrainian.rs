...
//!
//! Stage 1 wires the plain `BaseTagger` lookup (dictionary + `added`/`removed`
//! manual lists). The tagger's custom `additionalTags`/`getAnalyzedTokens`
//! heuristics (compound tagging, apostrophe/ґ/ї alternates, numerals, dates,
//! hashtags) land in stage 3.
...
/// Port of `UkrainianTagger`'s `BaseTagger` foundation
/// (`tagLowercaseWithUppercase = false`).
#[derive(Debug)]
...
impl UkrainianTagger {
    ...
    /// `BaseTagger.getAnalyzedTokens` for one word (plain lookup;
    /// `tagLowercaseWithUppercase = false`).
    pub fn tag_word(&self, word: &str) -> Vec<AnalyzedToken> {
        ...
        let _tagger_tokens_empty = l.len() == before;
        if !is_lowercase && !is_mixed {
            self.add_from_word_tagger(word, &lower_word, &mut l);
        }
        // `tagLowercaseWithUppercase = false` for Ukrainian: a lowercase word
        // is *not* retried with start-uppercase dictionary entries.
        if l.is_empty() {

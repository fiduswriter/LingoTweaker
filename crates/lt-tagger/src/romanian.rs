//! Romanian tagger: port of `org.languagetool.tagging.ro.RomanianTagger`,
//! which is a plain `BaseTagger` over the `romanian.dict` Morfologik
...
impl RomanianTagger {
    /// Load `ro/dictionaries/romanian.{dict,info}` plus the manual word lists.
    pub fn load(data_dir: &Path) -> Result<Self> {
        let dict_dir = data_dir.join("ro/dictionaries");
        let info = DictionaryInfo::load(&dict_dir.join("romanian.info"))?;
        let dict = Dictionary::load(&dict_dir.join("romanian.dict"), &info)?;
    ...
    /// `BaseTagger.getAnalyzedTokens` for one word (Romanian: no
    ...
    /// Tokenizer helper: `RomanianTagger.INSTANCE.tag([word]).get(0).isTagged()`.

//! Swedish tagger: port of `org.languagetool.tagging.sv.SwedishTagger`, a plain
//! `BaseTagger` over the `swedish.dict` Morfologik dictionary (no
...
impl SwedishTagger {
    /// Load `sv/dictionaries/swedish.{dict,info}` plus the manual word lists.
    pub fn load(data_dir: &Path) -> Result<Self> {
        let dict_dir = data_dir.join("sv/dictionaries");
        let info = DictionaryInfo::load(&dict_dir.join("swedish.info"))?;
        let dict = Dictionary::load(&dict_dir.join("swedish.dict"), &info)?;
    ...
    /// `BaseTagger.getAnalyzedTokens` for one word (Swedish: no
    ...
    /// Tokenizer helper: `SwedishTagger.INSTANCE.tag([word]).get(0).isTagged()`.

//! Port of `org.languagetool.synthesis.sv.SwedishSynthesizer` (+ the parts of
//! `BaseSynthesizer` it uses): inflected forms for a lemma and POS tag.
//!
//! The Swedish synthesizer is a plain `BaseSynthesizer` — no language
//! overrides — so this is the unmodified base lookup: `swedish_synth.dict`
//! keyed by `lemma|tag`, the manual `added.txt`/`removed.txt` synthesizers
//! and the tag list from `swedish_synth.dict_tags.txt`.
...
impl SwedishSynthesizer {
    /// Load from the vendored data directory (`sv/dictionaries`, `sv/words`).
    pub fn from_data(data_dir: &Path) -> Result<Self> {
        let dict_dir = data_dir.join("sv/dictionaries");
        let info = DictionaryInfo::load(&dict_dir.join("swedish_synth.info"))?;
        let dict = SynthDictionary::load(&dict_dir.join("swedish_synth.dict"), &info)?;
        let tag_text = lt_data::fs::read_to_string(dict_dir.join("swedish_synth_tags.txt"))
            .map_err(|e| CoreError::Data(format!("cannot read swedish_synth_tags.txt: {e}")))?;
        let mut possible_tags: Vec<String> = tag_text
            ...
            .map(str::to_string)
            .collect();
        let words_dir = data_dir.join("sv/words");
    ...
    /// file name for Swedish, so `getSpelledNumber` returns the numeral
    /// unchanged (`_spell_number_:feminine` therefore prepends `"feminine "`).
    /// `Roman.sor` is not vendored; no Swedish rule references the Roman tag.
    pub fn synthesize(
        &self,
        token: &AnalyzedToken,
        pos_tag: &str,

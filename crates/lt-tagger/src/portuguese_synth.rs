//! Port of `org.languagetool.synthesis.pt.PortugueseSynthesizer` (+ the parts
//! of `BaseSynthesizer` it uses): inflected forms for a lemma and POS tag,
//! with the Portuguese `pt.sor` number speller (`SOR_FILE_NAME`).
//!
//! `PortugueseSynthesizer` is a plain `BaseSynthesizer` — no `isException`
//! override — so this is the unmodified base lookup: `portuguese_synth.dict`
//! keyed by `lemma|tag`, the manual `added.txt`/`removed.txt` synthesizers and
//! the tag list from `portuguese_tags.txt`.
...
pub struct PortugueseSynthesizer {
...
impl PortugueseSynthesizer {
    /// Load from the vendored data directory (`pt/dictionaries`, `pt/words`).
    pub fn from_data(data_dir: &Path) -> Result<Self> {
        let dict_dir = data_dir.join("pt/dictionaries");
        let info = DictionaryInfo::load(&dict_dir.join("portuguese_synth.info"))?;
        let dict = SynthDictionary::load(&dict_dir.join("portuguese_synth.dict"), &info)?;
        let tag_text = lt_data::fs::read_to_string(dict_dir.join("portuguese_tags.txt"))
            .map_err(|e| CoreError::Data(format!("cannot read portuguese_tags.txt: {e}")))?;
        ...
        // `BaseSynthesizer` resolves `/pt/pt.sor`; when the resource is
        // missing Java's `Soros` stays null and the numeral is returned
        // unchanged (like Italian).
        let number_speller = lt_data::fs::read_to_string(data_dir.join("pt/pt.sor"))
            .ok()
            .map(|source| Soros::new(&source, "pt"));
        // `BaseSynthesizer.createRomanNumberer` reads the shared
        // `core/Roman.sor` (`/Roman.sor` in Java); the Portuguese
        // `RomanNumeralFilter` is the first rule that needs it.
    ...
    pub fn synthesize(
        ...
        if !pos_tag_regexp {
            let spelled = match pos_tag {
                ...
                // `Roman.sor` is vendored at `core/Roman.sor` (stage 3b).

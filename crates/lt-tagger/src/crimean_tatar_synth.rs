//! Port of `org.languagetool.synthesis.crh.CrimeanTatarSynthesizer` (+ the
//! parts of `BaseSynthesizer` it uses): inflected forms for a lemma and POS
//! tag. The Crimean Tatar synthesizer is a plain `BaseSynthesizer` — no
//! language overrides — so this is the unmodified base lookup:
//! `crimean_tatar_synth.dict` keyed by `lemma|tag`, the manual
//! `added.txt`/`removed.txt` synthesizers and the tag list from
//! `crimean_tatar_synth_tags.txt`.
...
pub struct CrimeanTatarSynthesizer {
...
impl CrimeanTatarSynthesizer {
    /// Load from the vendored data directory (`crh/dictionaries`, `crh/words`).
    pub fn from_data(data_dir: &Path) -> Result<Self> {
        let dict_dir = data_dir.join("crh/dictionaries");
        let info = DictionaryInfo::load(&dict_dir.join("crimean_tatar_synth.info"))?;
        let dict = SynthDictionary::load(&dict_dir.join("crimean_tatar_synth.dict"), &info)?;
        let tag_text = lt_data::fs::read_to_string(dict_dir.join("crimean_tatar_synth_tags.txt"))
            .map_err(|e| {
            CoreError::Data(format!("cannot read crimean_tatar_synth_tags.txt: {e}"))
        })?;
    ...
    /// `synthesize(AnalyzedToken, String, boolean)`. No Crimean Tatar rule
    /// uses the `_spell_number_` tags, so only the plain lookup is needed.
    pub fn synthesize(
        &self,
        token: &AnalyzedToken,
        pos_tag: &str,

//! Khmer tagger: port of `org.languagetool.tagging.km.KhmerTagger`, a plain
//! `BaseTagger` over the `khmer.dict` Morfologik dictionary (no
...
impl KhmerTagger {
    /// Load `km/dictionaries/khmer.{dict,info}` plus the manual word lists.
    pub fn load(data_dir: &Path) -> Result<Self> {
        let dict_dir = data_dir.join("km/dictionaries");
        let info = DictionaryInfo::load(&dict_dir.join("khmer.info"))?;
        let dict = Dictionary::load(&dict_dir.join("khmer.dict"), &info)?;
        let words_dir = data_dir.join("km/words");
    ...
    /// `BaseTagger.getAnalyzedTokens` for one word (Khmer: no
    ...
    /// Tokenizer helper: `KhmerTagger.INSTANCE.tag([word]).get(0).isTagged()`.

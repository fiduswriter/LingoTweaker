//! Tagalog tagger: port of `org.languagetool.tagging.tl.TagalogTagger`, a
//! plain `BaseTagger` over the `tagalog.dict` Morfologik dictionary
//! (CFSA2, ISO-8859-1) with no language override.
...
impl TagalogTagger {
    /// Load `tl/dictionaries/tagalog.{dict,info}` plus the manual word lists.
    pub fn load(data_dir: &Path) -> Result<Self> {
        let dict_dir = data_dir.join("tl/dictionaries");
        let info = DictionaryInfo::load(&dict_dir.join("tagalog.info"))?;
        let dict = Dictionary::load(&dict_dir.join("tagalog.dict"), &info)?;
        let words_dir = data_dir.join("tl/words");
    ...
    /// `BaseTagger.getAnalyzedTokens` for one word (Tagalog: no
    ...
    /// Tokenizer helper: `TagalogTagger.tag([word]).get(0).isTagged()`.

//! Malayalam tagger: port of `org.languagetool.tagging.ml.MalayalamTagger`, a plain
//! `BaseTagger` over the `malayalam.dict` Morfologik dictionary (no
//! `additionalTags` heuristics, no word-tokenizer overrides). The in-tree
//! `added.txt`/`added_custom.txt`/`removed.txt`/`removed_custom.txt` are
//! comment-only headers, so the manual tagger lists are empty.
...
impl MalayalamTagger {
    /// Load `ml/dictionaries/malayalam.{dict,info}` plus the manual word lists.
    pub fn load(data_dir: &Path) -> Result<Self> {
        let dict_dir = data_dir.join("ml/dictionaries");
        let info = DictionaryInfo::load(&dict_dir.join("malayalam.info"))?;
        let dict = Dictionary::load(&dict_dir.join("malayalam.dict"), &info)?;
        let words_dir = data_dir.join("ml/words");
    ...
    /// `BaseTagger.getAnalyzedTokens` for one word (Malayalam: no
    ...
    /// Tokenizer helper: `MalayalamTagger.INSTANCE.tag([word]).get(0).isTagged()`.

//! Italian tagger: port of `org.languagetool.tagging.it.ItalianTagger`,
//! which is a plain `BaseTagger` over the `italian.dict` Morfologik
//! dictionary (no `additionalTags` heuristics, no apostrophe rewriting).
...
impl ItalianTagger {
    /// Load `it/dictionaries/italian.{dict,info}` plus the manual word lists.
    pub fn load(data_dir: &Path) -> Result<Self> {
        let dict_dir = data_dir.join("it/dictionaries");
        let info = DictionaryInfo::load(&dict_dir.join("italian.info"))?;
        let dict = Dictionary::load(&dict_dir.join("italian.dict"), &info)?;
    ...
    /// `BaseTagger.getAnalyzedTokens` for one word (Italian: no
    ...
    /// Tokenizer helper: `ItalianTagger.INSTANCE.tag([word]).get(0).isTagged()`.

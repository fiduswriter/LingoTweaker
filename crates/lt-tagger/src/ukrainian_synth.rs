//! Port of `org.languagetool.synthesis.uk.UkrainianSynthesizer` (+ the parts of
//! `BaseSynthesizer` it uses): inflected forms for a lemma and POS tag.
//!
//! `UkrainianSynthesizer` is a plain `BaseSynthesizer` with a `null` number
//! speller (`super(RESOURCE_FILENAME, TAGS_FILE_NAME, "uk")`), so
...
pub struct UkrainianSynthesizer {
...
impl UkrainianSynthesizer {
    /// Load from the vendored data directory (`uk/dictionaries`, `uk/words`).
    pub fn from_data(data_dir: &Path) -> Result<Self> {
        let dict_dir = data_dir.join("uk/dictionaries");
        let info = DictionaryInfo::load(&dict_dir.join("ukrainian_synth.info"))?;
        let dict = SynthDictionary::load(&dict_dir.join("ukrainian_synth.dict"), &info)?;
        let tag_text = lt_data::fs::read_to_string(dict_dir.join("ukrainian_tags.txt"))
            .map_err(|e| CoreError::Data(format!("cannot read ukrainian_tags.txt: {e}")))?;
    ...
    /// numeral unchanged (the Ukrainian `BaseSynthesizer` passes no Soros file).

//! Asturian tagger: port of `org.languagetool.tagging.ast.AsturianTagger`, a
//! plain `BaseTagger` over the `asturian.dict` Morfologik dictionary (no
...
impl AsturianTagger {
    /// Load `ast/dictionaries/asturian.{dict,info}` plus the manual word lists.
    pub fn load(data_dir: &Path) -> Result<Self> {
        let dict_dir = data_dir.join("ast/dictionaries");
        let info = DictionaryInfo::load(&dict_dir.join("asturian.info"))?;
        let dict = Dictionary::load(&dict_dir.join("asturian.dict"), &info)?;
        let words_dir = data_dir.join("ast/words");
    ...
    /// `BaseTagger.getAnalyzedTokens` for one word (Asturian: no
    ...
    /// Tokenizer helper: `AsturianTagger.INSTANCE.tag([word]).get(0).isTagged()`.

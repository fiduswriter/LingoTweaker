//! Port of `org.languagetool.synthesis.it.ItalianSynthesizer` (+ the parts of
//! `BaseSynthesizer` it uses): inflected forms for a lemma and POS tag.
//!
//! The Italian synthesizer is a plain `BaseSynthesizer` — no language
//! overrides — so this is the unmodified base lookup: `italian_synth.dict`
//! keyed by `lemma|tag`, the manual `added.txt`/`removed.txt` synthesizers
//! and the tag list from `italian_tags.txt`.
...
impl ItalianSynthesizer {
    /// Load from the vendored data directory (`it/dictionaries`, `it/words`).
    pub fn from_data(data_dir: &Path) -> Result<Self> {
        let dict_dir = data_dir.join("it/dictionaries");
        let info = DictionaryInfo::load(&dict_dir.join("italian_synth.info"))?;
        let dict = SynthDictionary::load(&dict_dir.join("italian_synth.dict"), &info)?;
        let tag_text = lt_data::fs::read_to_string(dict_dir.join("italian_tags.txt"))
            .map_err(|e| CoreError::Data(format!("cannot read italian_tags.txt: {e}")))?;
        let mut possible_tags: Vec<String> = tag_text
            ...
            .map(str::to_string)
            .collect();
        let words_dir = data_dir.join("it/words");
    ...
    /// file name for Italian, so `getSpelledNumber` returns the numeral
    /// unchanged (`_spell_number_:feminine` therefore prepends `"feminine "`).
    /// `Roman.sor` is not vendored; no Italian rule references the Roman tag.
    pub fn synthesize(
        &self,
        token: &AnalyzedToken,
        pos_tag: &str,

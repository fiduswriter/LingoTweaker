//! Port of `org.languagetool.synthesis.gl.GalicianSynthesizer` (+ the parts of
//! `BaseSynthesizer` it uses): inflected forms for a lemma and POS tag.
//!
//! `GalicianSynthesizer` is a plain `BaseSynthesizer` with a `null` number
//! speller (`super(RESOURCE_FILENAME, TAGS_FILE_NAME, "gl")`), so
...
pub struct GalicianSynthesizer {
...
impl GalicianSynthesizer {
    /// Load from the vendored data directory (`gl/dictionaries`, `gl/words`).
    pub fn from_data(data_dir: &Path) -> Result<Self> {
        let dict_dir = data_dir.join("gl/dictionaries");
        let info = DictionaryInfo::load(&dict_dir.join("galician_synth.info"))?;
        let dict = SynthDictionary::load(&dict_dir.join("galician_synth.dict"), &info)?;
        let tag_text = lt_data::fs::read_to_string(dict_dir.join("galician_tags.txt"))
            .map_err(|e| CoreError::Data(format!("cannot read galician_tags.txt: {e}")))?;
    ...
    /// numeral unchanged (Galician's `sorosFileName` is null).

//! Dumps raw French tagger readings in the same TSV format as
//! `scripts/oracle/fr/DumpTags.java` (tagger verification).
//!
//! Usage: cargo run -p lt-tagger --example dump_tags_fr -- <sentences.txt>
...
fn main() {
    ...
    let path = PathBuf::from(args.next().expect("usage: dump_tags_fr <sentences.txt>"));
    let data = lt_data::DataDir::discover().expect("data dir");
    let file = std::fs::File::open(&path).expect("open input");
    let tagger = lt_tagger::FrenchTagger::load(data.path()).expect("tagger");

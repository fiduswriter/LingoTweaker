...
- Inputs are the exact corpus extractions: en = incorrect, non-trigger
  examples of `en-examples.jsonl`; de/es/fr/pt/gl = all example texts (embedded
  newlines replaced by spaces; gl excludes the parallel-corpus `bitext.xml`,
  which the engine does not load as a rule file). pt runs Java/the Rust CLI on the default
  variant (pt-PT); the corpus also carries pt-BR/pt-AO/pt-MZ rule examples
  whose rules are inactive on both sides.
...
- The date filters are pinned in `scripts/ci/parity.sh`: en/de/es use
  ...
  their capture date `2026-09-19`, and ca/gl use `2026-09-20`.
- Expected state: de/es/it/nl/ca exactly 0 only-Java / 0 only-Rust / 0 field
  ...
  runs the affected languages, not a static list (all nine on shared
  ...
  (D-188…D-190); gl is 0/0/83 (D-192…D-198).

Regenerate one language after an intentional corpus change (Docker):

```sh
scripts/ci/update-golden.sh en|de|es|fr|it|pt|nl|ca|gl   # rewrites <lang>-full.java.tsv
```

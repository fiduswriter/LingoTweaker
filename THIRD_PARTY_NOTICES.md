...
| Swedish POS/synthesis dictionaries, rebuilt from SALDO (`SwedishTagger`/`SwedishSynthesizer`) | `sv/dictionaries/swedish.dict`, `swedish_synth.dict`, `swedish_synth_tags.txt` | CC BY 4.0 (Språkbanken SALDO) | [SALDO distribution page](https://sprakbanken.se/en/resources/saldo) (download table: CC-BY-4.0); data source [saldom.xml](https://svn.spraakbanken.gu.se/sb-arkiv/pub/lexikon/saldom.xml), rebuilt in-tree by `tools/sv-dict/build-sv-tagger.py` (see `data/sv/README.md`) | Confirmed (CC BY 4.0 stated on the official SALDO resource page) |
| Swedish dictionary metadata files (DSSO) | `sv/dictionaries/swedish.info`, `swedish_synth.info` | LGPL-2.1-or-later (DSSO) | upstream `resource/sv/README.txt` (= `sv/words/README.txt`; Initial Developer Göran Andersson) | Confirmed |
...
| Nynorsk Hunspell dictionary | `nn/hunspell/nn_NO.dic` | CC BY 4.0 (Nasjonalbiblioteket/Ordbanken full forms) + CLARIN PUB+BY (Nynorskordboka nyordsliste 2018–2024, CLARINO Bergen) | [LibreOffice dictionaries `no/README_NO.txt`](https://github.com/LibreOffice/dictionaries/blob/master/no/README_NO.txt) (v3.0, same package as `nb_NO.dic`) | Confirmed (same source and licence situation as the confirmed `nb_NO.dic` vendoring); attribution summary in the README |
| Nynorsk affix rules | `nn/hunspell/nn_NO.aff` | GPL-2.0 (spell-norwegian project) | same README and `no/COPYING` | Confirmed; vendored unmodified (no `CHECKCOMPOUNDTRIPLE`/`SIMPLIFIEDTRIPLE` in the file, unlike `nb_NO.aff`) |
...
| Hand-authored `no`/`nrd`/`nn`/`gn` rules, word lists and speller additions | `no/rules/**`, `no/hunspell/ignore.txt`, `no/hunspell/prohibit.txt`, `nrd/rules/**`, `nrd/words/**`, `nrd/hunspell/ignore.txt`, `nn/rules/**`, `nn/hunspell/ignore.txt`, `nn/hunspell/repetition_exceptions.txt`, `gn/rules/**`, `gn/hunspell/ignore.txt` | LGPL-2.1-or-later | project `LICENSE` | Confirmed |
...
| Apertium nno-nob bilingual dictionary (candidate source only) | not vendored; candidates derived for `nn/rules/bokmaal_forms.txt` (LGPL-2.1-or-later list rule) | GPL-2.0 | [apertium/apertium-nno-nob](https://github.com/apertium/apertium-nno-nob), file `apertium-nno-nob.nno-nob.dix` (retrieved 2026-09-24) | Confirmed; **not a derived distribution of the dix**: only single-word translation pairs were used as *candidate hints*, each kept pair was re-verified against the independently licensed `nb_NO.dic`/`nn_NO.dic` spellers, hand-picked, and recorded in the file header; the shipped file is an original curated work, no dix text is copied |
...
| Japanese rules + Lindera segmenter | `ja/rules/grammar.xml`; segmenter `lindera`/`lindera-dictionary` 6.0.0 (patched copy in `vendor/lindera-dictionary`, wrapper `crates/lt-lindera`) | rules: LGPL-2.1-or-later (LanguageTool resource); Lindera engine: MIT | upstream `ja` module pom (LGPL-2.1); Lindera repository `LICENSE`; `vendor/README.md` | Confirmed |
| Japanese IPADIC dictionary (generated) | `ja/dictionary/*` (gitignored; generated by `tools/lindera/build-dict.py`) | mecab-ipadic 2.7.0-20070801 NAIST/ICOT notice (permissive, notice-retention) | Lindera release asset `lindera-ipadic-6.0.0.zip` bundled `NOTICE.txt`, extracted to `ja/dictionary/NOTICE.txt` | Confirmed |
| Chinese rules | `zh/rules/grammar.xml` | LGPL-2.1-or-later (LanguageTool resource) | upstream `zh` module pom (LGPL-2.1) | Confirmed |
| Chinese jieba dictionary (generated) | `zh/dictionary/*` (gitignored; generated by `tools/lindera/build-dict.py`) | jieba `dict.txt.big` MIT + CC-CEDICT data CC BY-SA 4.0 | Lindera release asset `lindera-jieba-6.0.0.zip` bundled `NOTICE.txt`, extracted to `zh/dictionary/NOTICE.txt` | Confirmed; owner note: CC-BY-SA-4.0 share-alike data in an LGPL distribution (like the German POS dictionary) |
